(function() {
  ...
  Game = function(eventSink, randy) {
    var player_actions_state;
    player_actions_state = function(player) {
      return {
        name: "player_actions",
        player: player,
        actions_remaining: 4,
    ...
    this.findLocation = function(locationName) {
      return _.findWhere(this.situation.locations, {
        name: locationName
      });
    };
    this.findDisease = function(diseaseName) {
      return _.findWhere(this.situation.diseases, {
        name: diseaseName
      });
    };
    this.findDiseaseByLocation = function(locationName) {
      var diseaseName;
      diseaseName = this.findLocation(locationName).disease;
      return this.findDisease(diseaseName);
    };
    this.findPlayer = function(playerId) {
      return _.findWhere(this.situation.players, {
        id: playerId
      });
    };
    ...
    this.drawPlayerCard = function(player) {
      var card;
      card = this.situation.player_cards_draw.shift();
      eventSink.emit({
    ...
    this.infect = function(loc, dis, num) {
      var max_infections, self, _infect;
      _infect = function(locs, dis, out) {
        var cur_loc, disease, location;
      ...
      max_infections = 3;
      self = this;
      return _infect(_.times(num, function() {
        return loc;
      }), dis, []);
    };
    ...
    this.setup = function(gameDef, players, settings) {
      var initialState, nDraw, roles, self, setupPlayerCards;
      setupPlayerCards = function() {
        var cards, chunkSize, chunks, counts, initialDeal, larger, n, nCards, nEpidemics, nReserved;
        ...
        if (nEpidemics > 0) {
          ...
          counts = _.times(nEpidemics, function(index) {
            return chunkSize + (index < larger ? 1 : 0);
          });
          chunks = _.map(counts, function(count) {
            var chunk;
            chunk = [this.index, this.index + count];
            this.index += count;
            return chunk;
          }, {
            index: nReserved
          ...
          return _.reduce(chunks, function(memo, chunk, index) {
            ...
            return memo.concat(cards.slice(chunk[0], where)).concat([
              {
                type: "epidemic",
                number: index
              }
      ...
      roles = _.map(gameDef.roles, function(role) {
        return role.name;
      });
      roles = randy.sample(roles, players.length);
      initialState.players = _.map(_.zip(players, roles), function(arr) {
        var player;
        player = _.object(["id", "role"], arr);
        player.location = gameDef.starting_location;
        player.hand = [];
        return player;
      });
      initialState.research_centers.push({
        location: gameDef.starting_location
      });
    ...
    this.resumeDrawPlayerCards = function() {
      var parent;
      parent = this.situation.state.parent;
    ...
    this.is_valid_player = function(player) {
      var playerObject;
      playerObject = this.findPlayer(player);
      if (!playerObject) {
    ...
    this.check_action_prerequisites = function(player, action) {
      var cards, destination, disease, from, location, movedPlayerObject, origin, self, source, thePlayer, to;
      ...
      switch (action.name) {
        ...
        case "action_discover_cure":
          ...
          cards = _.map(action.cards, function(card) {
        ...
        case "draw_infection_card":
          ...
          if (!this.drawInfection(1)) {
            return true;
          }
          break;
        case "discard_player_card":
          thePlayer = this.findPlayer(player);
          if (!_.find(thePlayer.hand, function(card) {
    ...
    this.eventRequriesApproval = function(eventName) {
      var eventsThatRequireApproval;
      eventsThatRequireApproval = ["action_drive", "action_charter_flight", "action_direct_flight", "action_shuttle_flight", "action_converge", "special_airlift"];
    ...
    this.medicEndMoveSpecialEffect = function() {
      var cured, location, medic;
      medic = _.find(this.situation.players, function(player) {
        return player.role === "Medic";
      });
      if (medic) {
        location = this.findLocation(medic.location);
        cured = _.filter(this.situation.diseases, function(disease) {
          return disease.status === "cure_discovered";
        });
    ...
    this.updateEradicatedDiseaseList = function() {
      ...
      eradicated = _.filter(this.situation.diseases, function(disease) {
    ...
    this.getCard = function(hand, attribute, targetToMatch) {
      return _.find(hand, function(card) {
        return card[attribute] === targetToMatch;
      });
    };
    ...
    this.performRegularAction = function(thePlayer, playerSelected, approved, player, action) {
      var card, cards, disease, from, location, number, other, self, to;
      switch (action.name) {
        case "action_pass":
          break;
        case "action_drive":
        ...
        case "action_build_research_center":
          ...
          this.situation.research_centers.push({
            location: thePlayer.location
          });
    ...
    this.performSpecialAction = function(playerSelected, playerSelectedObject, card, player, action) {
      if (action.name === "special_airlift") {
        ...
        this.situation.infection_cards_discard = _.filter(this.situation.infection_cards_discard, function(card) {
    ...
    this.performRegulatoryCardAction = function(player, action) {
      var card, cards, index, nextPlayer, players, thePlayer;
      thePlayer = this.findPlayer(player);
      switch (action.name) {
        case "draw_player_card":
          if (this.situation.state.draws_remaining === 0) {
            return this.startInfectionPhase(player);
          } else if (this.situation.state.name === "draw_player_cards") {
            return this.enterDrawState(player, this.situation.state.draws_remaining);
          } else {
            return this.emitStateChange();
          }
          break;
        case "discard_player_card":
          card = _.find(thePlayer.hand, function(card) {
            return _.isEqual(card, action.card);
          });
          this.discardPlayerCard(player, card);
          if ((this.situation.state.name === "hand_limit_exceeded") && (this.situation.state.player === player) && (thePlayer.hand.length <= this.situation.max_player_cards)) {
            if (this.situation.state.parent.name === "player_actions") {
              return this.resumePlayerActions();
            } else {
              return this.resumeDrawPlayerCards();
            }
          }
          break;
        case "increase_infection_intensity":
    ...
    this.act = function(player, action) {
      var action_result, activeDisease, approved, card, playerSelected, playerSelectedObject, thePlayer;
      ...
      activeDisease = _.find(this.situation.diseases, function(disease) {
